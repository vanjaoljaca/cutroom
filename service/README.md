...
The Node child owns both the editor/API at `127.0.0.1:4173` and the current same-Wi-Fi phone review at the machine's `en0` address on port `4174`. The review route serves the latest dated manifest below `/Volumes/VanjaOljacaX/Cutroom/reviews` as a full-viewport vertical swipe feed with byte-range MP4 playback. It contains no localhost media URLs.
